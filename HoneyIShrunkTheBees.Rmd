...
Here I outline the data import process, review the data, clean it, tidy it, and structure it to suit our needs. Please see the [first appendix](add link to the first appendix in GitHub) at the end for more details on this process. The MySQL queries can also be found there.
...
#This report displays only one year of data on a honey production census
honeycensus2012 <- sqlQuery(con, "select * from cahoneycensus_county_2012")
kable(summary(honeycensus2012))

#load the data
#honey_county <- (sqlQuery(con, "select * from honeycounty"))
#honey_state <- (sqlQuery(con, "select * from honeystate"))

#kable(head(honey_county))
#kable(head(honey_state))
```
...
[Appendix 1: Data Merging in MySQL - NASS and ](http://www.github.com/raw file to the appendix)
