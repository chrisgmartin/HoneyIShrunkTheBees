...
Other data sets we will need come from the City University of New York's Masters of Data Analytics Program (CUNY MSDA Program) course IS 606 - Statistics and Probability for Data Analytics. These custom files assist in the 

```{r}
load(url("ADD GITHUB LINK TO THE RDATA PACKAGE FOR NC.RDATA"))
```
...
StateProd2 <- StateProd2 %>% 
  group_by(Year) %>% 
  summarise(value = sum(value))
plot_ly(StateProd, x = Year, y = value, text = paste("Year: ", Year), mode = "markers", color = value, size = value)
```
...
But with so many different Data Items in our data set we're likely to get one that's fairly riduculous (I'm looking at you **HORTICULTURE TOTALS, WHOLESALE, LANDSCAPE REDISTRIBUTION YARDS - SALES, MEASURED IN $**!) Let's use one we've looked at before: **HORTICULTURE TOTALS, (EXCL CUT TREES), IN THE OPEN - ACRES IN PRODUCTION**.

First we can see if the relationship appears linear. I'm going to create a new data frame and remove all null values as well (changing them to zero will skew our data):

```{r}
which(colnames(honey_state3) =='CHEMICAL TOTALS - EXPENSE, MEASURED IN $')
Honey_Chems <- honey_state3[,c(1,2,3,39)]
colnames(Honey_Chems) <- c("Year", "State", "Chems","Honey")
Honey_Chems <- Honey_Chems[complete.cases(Honey_Chems[,3:4]),]

#calculate correlation
cor(Honey_Chems$Honey, Honey_Chems$Chems)
```

We can also check it on a plot:

```{r}
ggplot(Honey_Chems, aes(x=Chems, y=Honey)) + geom_line()
```

But we don't learn a whole lot other than there seems to be a higher grouping in the low side of Chemical spending and lbs of honey production per colony.

```{r}
hc1 <- lm(Chems ~ Honey, data=Honey_Chems)
summary(hc1)
plot(hc1$residuals ~ Honey_Chems$Honey)
abline(h=0, lty=3)
```
