...
Here I outline the data import process, review the data, clean it, tidy it, and structure it to suit our needs. Please see [Appendix 1: Data Merging in MySQL](https://github.com/chrisgmartin/HoneyIShrunkTheBees/tree/master/Appendix) at the end for more details on this process. The MySQL queries can also be found there. All of the data has also been saved in all of their forms (finished and unfinished) in the [GitHub folder: Data](https://github.com/chrisgmartin/HoneyIShrunkTheBees/tree/master/Data)
...
Using the FIPS County codes [reference table](https://www.census.gov/geo/reference/codes/cou.html) which has been saved and stored locally using MySQL, we can see if the ANSI codes match the FIPS codes:
...
There are a lot more data items (87 more to be exact) in the state set than in the county set, yet the county set has more variables/row (33,548). This could mean a number of things but it certainly means that the starting point in the analysis will be counties.
...
honey_state2 <- honey_state[c(1,5,6,7,10,11,16,17,20)]
...
*Important note*: R currently cannot properly store integers greater than 2,147,483,647 (I was getting an error which [pointed me to this link](http://stackoverflow.com/questions/14589354/struggling-with-integers-maximum-integer-size) ). As a result, I have fixed all values greater than 2,147,483,647 to be equal to 2,147,483,647 in the MySQL query. This will absolutely lead to errors in our data, and in a real-life situation this would never be a suggestion to recommened, ever. As my purpose is more about the journey than the destination (ahem), I this is a necessary evil I had to choose, we may not even be affected by it in the end. But in real-life, we will always be affected by the assumptions we make.
